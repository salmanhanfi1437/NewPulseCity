...
export default {
  ...
  viewQR: (page?: number, limit?: number, search?: string) =>apiClient.get(`qr-management/distributor/view?page=${page}&limit=${limit}?&search=${search}`),
notifications: (page: number, limit: number, type: string) =>
  ...
  getAppVersions : () => apiClient.get(`app-version`),   
